...
function QuizzList() {
    ...
    const [columns] = useState<TableColumnModel[]>([
        { field: 'thumbnailUrl', label: 'Header', iconASC: faSortAlphaAsc, iconDESC: faSortAlphaDesc, isEnum: false, enum: null, sortabled: true },
        { field: 'title', label: 'Title', iconASC: faSortAlphaAsc, iconDESC: faSortAlphaDesc, isEnum: false, enum: null, sortabled: true },
        { field: 'description', label: 'Description', iconASC: faSortAmountAsc, iconDESC: faSortAmountDesc, isEnum: false, enum: null, sortabled: true },
        { field: 'duration', label: 'Duration', iconASC: faSortAlphaAsc, iconDESC: faSortAlphaDesc, isEnum: false, enum: null, sortabled: true },
        { field: 'numberOfAnswers', label: 'Answer', iconASC: faSortAlphaAsc, iconDESC: faSortAlphaDesc, isEnum: false, enum: null, sortabled: true },
    ...
    const searchData = useCallback(async () => {
        try {
            ...
            // const response: any = await QuizzService.search(filter);

            const response: any = await QuizzService.getAll();
            setData(response);
    ...
    const onDelete = async (item: any) => {
        try {
            const response = await QuizzService.remove(item.id);
        ...
        <section className="w-full">
            ...
            {/* <div className="card border border-slate-300 rounded-md">
                <div className="card-header p-3">
                    <h1 className="text-2xl font-semibold">Quizz List</h1>
                </div>
            ...
            <TablePagination data={data} defaultOrderBy={'title'} pageInfo={pageInfo} columns={columns} onEdit={onEdit} onDelete={onDelete} onSearch={onSearch} />

            {/* Details Component */}
            {/* <div id="detail-form" ref={detailForm}>
                {isShowDetail && (<QuizzDetail item={selectedItem} onCancel={() => onCancelDetail()} />)}
            </div> */}
